...
$pdf->Cell(25.5,0.7,"Laporan Transaksi Masuk",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(3, 0.8, 'No. Nota', 1, 0, 'C');
$pdf->Cell(7, 0.8, 'Tanggal Masuk', 1, 0, 'C');
$pdf->Cell(4, 0.8, 'ID Distributor', 1, 0, 'C');
$pdf->Cell(4.5, 0.8, 'ID Petugas', 1, 0, 'C');
$pdf->Cell(4.5, 0.8, 'Total', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'Ket.', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
$query = mysqli_query($connect, "select * from tblbrgmasuk");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['no_nota'],1, 0, 'C');
	$pdf->Cell(7, 0.8, $lihat['tgl_masuk'], 1, 0,'C');
	$pdf->Cell(4, 0.8, $lihat['id_distributor'],1, 0, 'C');
	$pdf->Cell(4.5, 0.8, $lihat['id_petugas'], 1, 0,'C');
	$pdf->Cell(4.5, 0.8, $lihat['total'],1, 0, 'C');
	$pdf->Cell(2, 0.8, $lihat['ket'], 1, 1,'C');
